...
-- Last update : Thu Feb  5 14:52:51 2026
...
entity ft232_interface is
  port (
    ...
    -- Bridge dispatcher side RX USB -> FPGA
    rx_data  : out std_logic_vector(7 downto 0);
    rx_valid : out std_logic;
    rx_ready : in std_logic;

    -- Bridge dispatcher side TX FPGA -> USB
...
begin
  ...
  ft_data <= reg_fsm.data_out when reg_fsm.data_dir = '1' else
    (others => 'Z');
    ...
    if (reset_n = '0') then
      ...
      case reg_fsm.state is
        when IDLE =>
          ...
          if ((ft_rxf_n = '0') and (rx_ready = '1')) then
            reg_fsm.ft_oe_n <= '0';
            reg_fsm.state   <= RX_OE;

          elsif (ft_txe_n = '0') and (tx_valid = '1') then
            reg_fsm.data_dir <= '1';
        ...
        when RX_RD =>
          ...
          if ((ft_rxf_n = '1') or (rx_ready = '0')) then
            reg_fsm.ft_oe_n <= '1';
            reg_fsm.ft_rd_n <= '1';
          else
            reg_fsm.ft_oe_n  <= '0';
        ...
        when TX =>
          if (ft_txe_n = '1') or (tx_valid = '0') then
            reg_fsm.ft_wr_n <= '1';
          else
            reg_fsm.data_out <= tx_data;
            reg_fsm.data_dir <= '1';
